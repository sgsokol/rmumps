          ...
          idx_t cnvtxs, size_t *r_nunmatched, size_t maxdegree)
{
  ...
  /*IFSET(ctrl->dbglvl, METIS_DBG_COARSEN, rcpp_warning("IN: nunmatched: %zu\t", * nunmatched)); */
  ...
  /* IFSET(ctrl->dbglvl, METIS_DBG_COARSEN, rcpp_warning("OUT: nunmatched: %zu\n", nunmatched)); */
          ...
          idx_t cnvtxs, size_t *r_nunmatched, size_t maxdegree)
{
  ...
  /*IFSET(ctrl->dbglvl, METIS_DBG_COARSEN, rcpp_warning("OUT: ncand: %zu, nunmatched: %zu\n", ncand, nunmatched)); */
...
void PrintCGraphStats(ctrl_t *ctrl, graph_t *graph)
{
  ...
  rcpp_warning("%10"PRIDX" %10"PRIDX" %10"PRIDX" [%"PRIDX"] [", 
      graph->nvtxs, graph->nedges, isum(graph->nedges, graph->adjwgt, 1), ctrl->CoarsenTo);

  for (i=0; i<graph->ncon; i++)
    rcpp_warning(" %8"PRIDX":%8"PRIDX, ctrl->maxvwgt[i], graph->tvwgt[i]);
  rcpp_warning(" ]\n");
}
