         ...
         real_t ffactor, idx_t omode)
{
  ...
  if (ctrl->dbglvl&METIS_DBG_REFINE) {
     rcpp_warning("%s: [%6"PRIDX" %6"PRIDX"]-[%6"PRIDX" %6"PRIDX"], Bal: %5.3"PRREAL"," 
     ...
     if (ctrl->minconn) 
       rcpp_warning(", Doms: [%3"PRIDX" %4"PRIDX"]", imax(nparts, nads), isum(nparts, nads,1));
     rcpp_warning("\n");
  }
  ...
  *======================================================================*/
  for (pass=0; pass<niter; pass++) {
    ...
    for (nmoved=0, iii=0;;iii++) {
      ...
      *======================================================================*/
      ...
      IFSET(ctrl->dbglvl, METIS_DBG_MOVEINFO, 
          rcpp_warning("\t\tMoving %6"PRIDX" to %3"PRIDX". Gain: %4"PRIDX". Cut: %6"PRIDX"\n", 
    ...
    if (ctrl->dbglvl&METIS_DBG_REFINE) {
       rcpp_warning("\t[%6"PRIDX" %6"PRIDX"], Bal: %5.3"PRREAL", Nb: %6"PRIDX"."
  ...
  for (pass=0; pass<niter; pass++) {
    ...
    for (nmoved=0, iii=0;;iii++) {
      ...
      *======================================================================*/
      ...
      IFSET(ctrl->dbglvl, METIS_DBG_MOVEINFO, 
          rcpp_warning("\t\tMoving %6"PRIDX" from %3"PRIDX" to %3"PRIDX". "
         ...
         real_t ffactor, idx_t omode)
{
  ...
  if (ctrl->dbglvl&METIS_DBG_REFINE) {
     rcpp_warning("%s: [%6"PRIDX" %6"PRIDX" %6"PRIDX"], Bal: %5.3"PRREAL"(%.3"PRREAL")," 
