...
pdbf *gk_readpdbfile(char *fname) { /* {{{ */
  ...
  int nresatoms;
	...
	/* rcpp_warning("File has coordinates for %d atoms in %d residues\n",atoms,residues); */
	toFill->natoms      = atoms;
	toFill->ncas        = cas;
	toFill->nbbs        = bbs;
  ...
  toFill->cm[res-1].z = avgz/nresatoms;
	/* Begin test code */
	if(cas != residues) {
		rcpp_warning("Number of residues and CA coordinates differs by %d (!)\n",residues-cas);
		if(cas < residues) {
...
void gk_showcorruption(pdbf *p) {
	...
	if(corruption&CRP_ALTLOCS)
		rcpp_warning("Multiple coordinate sets for at least one atom\n");
	if(corruption&CRP_MISSINGCA) 
		rcpp_warning("Missing coordiantes for at least one CA atom\n");
	if(corruption&CRP_MISSINGBB) 
		rcpp_warning("Missing coordiantes for at least one backbone atom (N,CA,C,O)\n");
	if(corruption&CRP_MULTICHAIN) 
		rcpp_warning("File contains coordinates for multiple chains\n");
	if(corruption&CRP_MULTICA) 
		rcpp_warning("Multiple CA atoms found for the same residue (could be alternate locators)\n");
	if(corruption&CRP_MULTICA) 
		rcpp_warning("Multiple copies of backbone atoms found for the same residue (could be alternate locators)\n");
}
			...
			rcpp_warning("%-6s%5d%-1s%-4s%1c%3s%1s%1c%4d%1c%3s%8.3lf%8.3lf%8.3lf%6.2f%6.2f%6s%4s%2s%2s\n",
			linetype,aserial," ",aname,altLoc,rname," ",chainid,rserial,icode," ",x,y,z,opcy,tmpt," ",segId,element,charge); */
			...
			linetype,&aserial,aname,&altLoc,rname,&chainid,&rserial,&icode,&x,&y,&z,&opcy,&tmpt,element);
			rcpp_warning("%-6s%5d%-1s%-4s%1c%3s%1s%1c%4d%1c%3s%8.3lf%8.3lf%8.3lf%6.2f%6.2f%6s%6s\n",
			linetype,aserial," ",aname,altLoc,rname," ",chainid,rserial,icode," ",x,y,z,opcy,tmpt," ",element); */
