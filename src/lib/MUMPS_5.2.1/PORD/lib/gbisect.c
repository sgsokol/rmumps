...
{ graph_t *G;
  ...
  rcpp_warning("\n#nodes %d, #edges %d, totvwght %d\n", G->nvtx, G->nedges >> 1,
         G->totvwght);
  rcpp_warning("partition weights: S %d, B %d, W %d\n", Gbisect->cwght[GRAY],
         Gbisect->cwght[BLACK], Gbisect->cwght[WHITE]);
  for (u = 0; u < G->nvtx; u++)
   { count = 0;
     rcpp_warning("--- adjacency list of node %d (weight %d, color %d)\n", u,
            G->vwght[u], Gbisect->color[u]);
     istart = G->xadj[u];
     istop = G->xadj[u+1];
     for (i = istart; i < istop; i++)
      { v = G->adjncy[i];
        rcpp_warning("%5d (color %2d)", v, Gbisect->color[v]);
        if ((++count % 4) == 0)
          rcpp_warning("%s", "\n");
      }
     if ((count % 4) != 0)
...
{ PORD_INT *xadj, *adjncy, *vwght, *color, *cwght;
  ...
  err = FALSE;
  rcpp_warning("checking separator of induced subgraph (S %d, B %d, W %d)\n",
   ...
   { istart = xadj[u];
     ...
     switch(color[u])
      { case GRAY:                /* is it a minimal separator? */
          ...
          if (!((a) && (b)))
            rcpp_warning("WARNING: not a minimal separator (node %d)\n", u);
          break;
        case BLACK:               /* is it realy a separator? */
          ...
          for (i = istart; i < istop; i++)
           { v = adjncy[i];
             if (color[v] == WHITE)
              { rcpp_warning("ERROR: white node %d adjacent to black node %d\n", u,v);
                err = TRUE;
              }
           }
          break;
        case WHITE:
          ...
          break;
        default:
          rcpp_warning("ERROR: node %d has unrecognized color %d\n", u, color[u]);
          err = TRUE;
      }
   }
...
{ domdec_t *dd, *dd2;
  ...
  if (options[OPTION_MSGLVL] > 2)
    rcpp_warning("\t  0. dom.dec.: #nodes %d (#domains %d, weight %d), #edges %d\n",
  ...
  while ((dd->ndom > MIN_DOMAINS) && (i < MAX_COARSENING_STEPS)
   ...
   { shrinkDomainDecomposition(dd, options[OPTION_NODE_SELECTION3]);
     ...
     if (options[OPTION_MSGLVL] > 2)
       rcpp_warning("\t %2d. dom.dec.: #nodes %d (#domains %d, weight %d), #edges %d"
  ...
  if (options[OPTION_MSGLVL] > 2)
    rcpp_warning("\t %2d. dom.dec. sep.: S %d, B %d, W %d [cost %7.2f]\n",
...
#ifdef DEBUG
  rcpp_warning("Dulmage-Mendelsohn decomp. computed\n"
...
#ifdef DEBUG
     rcpp_warning("%s", "exchange SI with BX\n");
#endif
...
#ifdef DEBUG
     rcpp_warning("%s", "exchange SR with BR\n");
#endif
  ...
  do
   { /* ---------------------------------------------------------------
     ...
     if ((options[OPTION_MSGLVL] > 2) && (a))
       rcpp_warning("\t separator smoothed: S %d, B %d, W %d [cost %7.2f]\n",
              cwght[GRAY], cwght[BLACK], cwght[WHITE],
