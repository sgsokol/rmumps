...
{ PORD_INT *xadj, *adjncy, *vwght, *vtype, *color, *cwght;
  ...
  err = FALSE;
  rcpp_warning("checking separator of domain decomposition (S %d, B %d, W %d)\n",
     ...
     { nBdom = nWdom = 0;
       ...
       switch(color[u])
        { case GRAY:
            checkS += vwght[u];
            if ((nBdom == 0) || (nWdom == 0))
              rcpp_warning("WARNING: multisec %d belongs to S, but nBdom = %d and "
                     "nWdom = %d\n", u, nBdom, nWdom);
            break;
          case BLACK: 
            checkB += vwght[u];
            if (nWdom > 0)
             { rcpp_warning("ERROR: black multisec %d adjacent to white domain\n", u);
               err = TRUE;
             }
            break;
          case WHITE:
            checkW += vwght[u];
            if (nBdom > 0)
             { rcpp_warning("ERROR: white multisec %d adjacent to black domain\n", u);
               err = TRUE;
             }
            break;
          default:
            rcpp_warning("ERROR: multisec %d has unrecognized color %d\n", u,
                   color[u]);
            err = TRUE;
        }
     }
    ...
    /* sum up size of white/black domains */
      ...
      switch(color[u])
       { case BLACK:
         ...
         default:
           rcpp_warning("ERROR: domain %d has unrecognized color %d\n", u, color[u]);
           err = TRUE;
       }
  ...
  if ((checkS != cwght[GRAY]) || (checkB != cwght[BLACK])
     || (checkW != cwght[WHITE]))
   { rcpp_warning("ERROR in partitioning: checkS %d (S %d), checkB %d (B %d), "
...
#ifdef DEBUG
        rcpp_warning(" B2W case (1): (via multisec %d) removing domain %d from "
               "w_bucket\n", u, v);
#endif
        ...
        for (j = jstart; j < jstop; j++)
         { v = adjncy[j];
           if (vtype[v] == 1)
            {
#ifdef DEBUG
              rcpp_warning(" B2W case (2): (via multisec %d) removing domain %d from "
                     "b_bucket\n", u, v);
#endif
      ...
      { for (j = jstart; j < jstop; j++)
         { v = adjncy[j];
           if ((tmp_color[v] == BLACK) && (vtype[v] == 1))
            {
#ifdef DEBUG
              rcpp_warning(" B2W case (3): (via multisec %d) removing domain %d from "
                     "b_bucket\n", u, v);
#endif
        ...
        for (j = jstart; j < jstop; j++)
         { v = adjncy[j];
           if (vtype[v] == 1)
            {
#ifdef DEBUG
              rcpp_warning(" B2W case (4): (via multisec %d) removing domain %d from "
                     "w_bucket\n", u, v);
#endif
...
#ifdef DEBUG
        rcpp_warning(" W2B case (1): (via multisec %d) removing domain %d from "
               "b_bucket\n", u, v);
#endif
        ...
        for (j = jstart; j < jstop; j++)
         { v = adjncy[j];
           if (vtype[v] == 1)
            {
#ifdef DEBUG
              rcpp_warning(" W2B case (2): (via multisec %d) removing domain %d from "
                     "w_bucket\n", u, v);
#endif
      ...
      { for (j = jstart; j < jstop; j++)
         { v = adjncy[j];
           if ((tmp_color[v] == WHITE) && (vtype[v] == 1))
            {
#ifdef DEBUG
              rcpp_warning(" W2B case (3): (via multisec %d) removing domain %d from "
                     "w_bucket\n", u, v);
#endif
        ...
        for (j = jstart; j < jstop; j++)
         { v = adjncy[j];
           if (vtype[v] == 1)
            {
#ifdef DEBUG
              rcpp_warning(" W2B case (4): (via multisec %d) removing domain %d from "
                     "b_bucket\n", u, v);
#endif
...
#ifdef DEBUG
  rcpp_warning("starting inner loop: b_bucket->nobj %d, w_bucket->nobj %d\n",
...
#ifdef DEBUG
     rcpp_warning("best black domain: %d, deltaS %d, deltaB %d, deltaW %d, "
...
#ifdef DEBUG
     rcpp_warning("best white domain: %d, deltaS %d, deltaB %d, deltaW %d, "
...
#ifdef DEBUG
  rcpp_warning(" domain %d removed from bucket\n", domain);
#endif
...
INNER_LOOP_END:
  ...
  /* ----------------------------------------------
     ...
     ---------------------------------------------- */
#ifdef DEBUG
  rcpp_warning(" INNER_LOOP_END (#pyhs. flips %d): S %d, B %d, W %d (%7.2f)\n",
