...
{ graph_t *G;
  ...
  for (u = 0; u < G->nvtx; u++)
   { count = 0;
     rcpp_warning("--- adjacency list of node %d (vtype %d, color %d, map %d\n",
            u, dd->vtype[u], dd->color[u], dd->map[u]);
     istart = G->xadj[u];
     istop = G->xadj[u+1];
     for (i = istart; i < istop; i++)
      { v = G->adjncy[i];
        rcpp_warning("%5d (vtype %2d, color %2d)", v, dd->vtype[v], dd->color[v]);
        if ((++count % 3) == 0)
          rcpp_warning("\n");
      }
     if ((count % 3) != 0)
...
{ PORD_INT *xadj, *adjncy, *vwght, *vtype;
  ...
  err = FALSE;
  rcpp_warning("checking domain decomposition (#nodes %d, #edges %d)\n",
  ...
  for (u = 0; u < nvtx; u++)
   { /* check node type */
     if ((vtype[u] != 1) && (vtype[u] != 2))
      { rcpp_warning("ERROR: node %d is neither DOMAIN nor MULTISEC\n", u);
        err = TRUE;
      }
     ...
     if ((vtype[u] == 1) && (dom > 0))
      { rcpp_warning("ERROR: domain %d is adjacent to other domain\n", u);
        err = TRUE;
      }
     if ((vtype[u] == 2) && (dom < 2))
      { rcpp_warning("ERROR: less than 2 domains adjacent to multisec node %d\n", u);
        err = TRUE;
      }
     if ((vtype[u] == 2) && (multi > 0))
      { rcpp_warning("ERROR: multisec %d is adjacent to other multisec nodes\n", u);
        err = TRUE;
      }
   }
  /* check number and weight of domains */
  if ((ndom != dd->ndom) || (domwght != dd->domwght))
   { rcpp_warning("ERROR: number/size (%d/%d) of domains does not match with those in"
